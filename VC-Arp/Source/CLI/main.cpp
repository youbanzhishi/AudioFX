// VC-Arp CLI - JUCE-based Command Line Tool
...
void printHelp(const char* progName) {
    std::cout << "VC-Arp CLI - Arpeggiator Audio Plugin Tool\n\n";
    std::cout << "Usage: " << progName << " <input.wav> <output.wav> [options]\n\n";
    std::cout << "Options:\n";
    std::cout << "  --help, -h           Show this help\n";
    std::cout << "  --bpm <30-300>       Tempo in BPM (default: 120)\n";
    std::cout << "  --transpose <-24-24> Transpose in semitones (default: 0)\n";
    std::cout << "  --gate <1-200>       Gate length %% (default: 100)\n";
    std::cout << "  --swing <0-100>      Swing amount %% (default: 0)\n";
    std::cout << "  --bypass <0|1>       Bypass processing (default: 0)\n\n";
    std::cout << "Examples:\n";
    std::cout << "  " << progName << " in.wav out.wav --bpm 140 --swing 30\n";
}
...
int main(int argc, char** argv) {
    ...
    std::cout << "VC-Arp CLI (JUCE Version)\n";
    ...
    if (args.count("--bpm")) {
        params.bpm = std::stof(args["--bpm"]);
        std::cout << "BPM: " << params.bpm << "\n";
    }

    if (args.count("--transpose")) {
        params.transpose = std::stoi(args["--transpose"]);
        std::cout << "Transpose: " << params.transpose << " semitones\n";
    }

    if (args.count("--gate")) {
        params.gate = std::stof(args["--gate"]);
        std::cout << "Gate: " << params.gate << " %\n";
    }

    if (args.count("--swing")) {
        params.swing = std::stof(args["--swing"]);
